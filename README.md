...
- Trade env vars: `OKX_TRADE_API_KEY`, `OKX_TRADE_API_SECRET`, `OKX_TRADE_PASSPHRASE`

The Binance USDT perpetual adapter remains as a legacy fallback for explicitly configured `binance_usdt_perp / ETHUSDT` flows. Real order submission is still blocked unless the existing real-order gates, kill switch checks, preflight checks, and explicit submit/confirmation flags pass.
...
- 默认对接 OKX USDT swap，处理签名请求、仓位快照、订单状态和保护单；Binance USDT perpetual 仅保留为显式 legacy fallback。
...
- 交易标的：`ETH`
- 合约：OKX USDT swap，默认 `ETH-USDT-SWAP`
- 杠杆：`10x`
- 主决策周期：`15m`
- 风险辅助周期：`5m`
- 默认运行模式：`shadow`
...
src/bot/
  ...
  exchange_adapter.py   # OKX 默认执行适配；Binance legacy fallback 适配
  okx_transport.py      # OKX REST 签名请求
  binance_transport.py  # Binance legacy REST 签名请求
...
Binance 交易变量仅用于显式配置 `binance_usdt_perp / ETHUSDT` 的 legacy fallback，不是默认实盘入口。
...
- 必须使用 `OKX_TRADE_API_KEY / OKX_TRADE_API_SECRET / OKX_TRADE_PASSPHRASE`
...
- 审计执行链路
- OKX adapter
- Binance legacy fallback adapter
- 保护止损维护
- 网络和 runtime 降级
...
- OKX transport/exchange adapter，以及 Binance legacy fallback adapter
