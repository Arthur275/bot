...
from scripts.quant_runtime_bridge import load_quant_outcome_writers
...
def run(*, args: argparse.Namespace, adapter: TradeHistoryAdapter | None = None) -> dict[str, Any]:
    ...
    writers = _load_outcome_writers(quant_root)
    summary = writers.upsert_decision_outcome(db_path, outcome)
    writers.write_decision_outcomes_summary(summary_path, summary)
    return {
        "status": "recorded",
...
def build_outcome(*, match: ManualCloseMatch, args: argparse.Namespace, generated_at: datetime) -> Any:
    writers = _load_outcome_writers(Path(args.quant_root))
    ...
    return writers.decision_outcome_factory(
...
def _load_outcome_writers(quant_root: Path):
    return load_quant_outcome_writers(quant_root=quant_root, fallback_quant_root=DEFAULT_QUANT_ROOT)
