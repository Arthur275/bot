...
def default_runtime_root() -> str:
    return str(runtime_root_for_repo(repo_root_from_file(__file__)))


def default_package_path() -> str:
    return str(candidate_execution_package_path(repo_root_from_file(__file__)))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--audit-log-path", default=str(real_order_worker_audit_path(repo_root_from_file(__file__))))
    parser.add_argument("--lock-path", default=str(real_order_worker_lock_path(repo_root_from_file(__file__))))
    parser.add_argument("--kill-switch-path", default=str(kill_switch_path(repo_root_from_file(__file__))))
...
def _execute_with_action_closure(
    *,
    ...
    if action == "exit":
        if _kill_switch_enabled(kill_switch_path):
            results.append(_synthetic_command_result(target="cleanup_open_algo_orders", reason="kill_switch_enabled_before_submit"))
            return results
